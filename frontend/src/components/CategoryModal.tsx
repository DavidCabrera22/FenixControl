...
export const CategoryModal = ({ isOpen, onClose, onSaved, initialData }: CategoryModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Error saving category form", err);
      ...
      >
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
          <div className="flex items-center gap-3">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${initialData ? 'bg-primary/10 text-primary' : 'bg-purple-100 text-purple-600 dark:bg-purple-900/30'}`}>
              <span className="material-symbols-outlined text-sm">{initialData ? 'edit' : 'category'}</span>
            </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="space-y-1">
            ...
            <input 
              ...
              placeholder="Ej. Alimentación, Transporte, Honorarios..." 
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Tipo de categoría</label>
            <select 
              value={formData.type}
